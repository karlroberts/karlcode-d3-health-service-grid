...
var d3_utils_1 = require("@karlcode/d3-utils");
var chartutils_1 = require("./chartutils");
// import shims from '../utils/polyfill/index'
var bestSquareTiles = d3_utils_1.default.geometry.bestSquareTiles;
var shims = d3_utils_1.default.polyfill;
function statusRings() {
    var prettyEmpty = constants_1.default.ft.emptyDataPretty;
    ...
    var svg = undefined;
    var my = function my(placeholder, thedata, dispatch) {
        ...
        var invisnodeNode = invisnode.node();
        ...
        var fsizeInEm = function (numChars, availWidth) {
        ...
        var strokeWidth = 3;
        //---------------- DISPATCHERS -------------
        dispatch.on('nodeHovered.gridChart', function (d, i, serviceName) {
            lights2(serviceName, true);
        });
        dispatch.on('nodeAbandoned.gridChart', function (d, i, serviceName) {
            lights2(serviceName, false);
        });
        //=====================
        function doBlur(isOn, fname) {
            if (!!!fname) {
                fname = "#B1";
            }
            if (isOn) {
        ...
        var colourNode = function (d) {
            return chartutils_1.default.colourizeNode(d.status);
        };
        colourNode.darker = function (d) {
            return this(d).darker();
        };
        colourNode.lighter = function (d) {
        ...
        function lights2(serviceName, isOn) {
            ...
            D3.selectAll(".lit-up").each(function (d, i) {
                ...
                var fillC = D3.rgb(el.style("fill"));
        ...
        serviceCirc.on('mouseover', function (d, i) {
        ...
        var arcs = squareG.selectAll('path.dependant').data(function (d, j) {
        ...
        var depArc = arcEnter.append("path")
            ...
            var padAngle = ((Math.PI / 180) * 3); //convert to deg then 3 degs
            var Arc = D3.arc();
        ...
        depArc.on('mouseover', function (d, i) {
            D3.select(this).style("cursor", "pointer"); //we have a click event so set pointer as indicator that we can be clicked
        ...
        var serviceLabel = squareG.append('text')
            ...
            return (!!sname) ? chartutils_1.default.fixWordLength(sname, constants_1.default.serviceNameLength) : chartutils_1.default.fixWordLength("??", constants_1.default.serviceNameLength);
        })
            ...
            var tlen = constants_1.default.serviceNameLength;
...
exports.statusRings = statusRings;
